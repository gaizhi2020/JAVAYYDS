...
二叉搜索树(Binary Search Tree), 又称二叉排序树,每个节点包含健值key,数据data,左子节点指针,右子节点指针;

特点:

左右子树也分别是二叉搜索树

左子树的所有key值都小于根节点key值

右子树的所有key值都大于根节点key值

二叉搜索树可以为一棵空树

一般来说,树中每个节点的key值都不相等,但根据需求,可以将相同的key值插入树中



**AVL树:**

也称平衡二叉搜索树,通过一定机制保证二叉搜索树的平衡,平衡二叉搜索树的查询效率更高
...
逆时针旋转,右孩子节点成为根节点,自己成为自己的左节点



**右旋:**

顺时针旋转,左孩子节点成为根节点,自己成为自己的右节点



### 18.7.2 红黑树:

是一种自平衡的二叉查找树,都在删除或者插入时通过旋转操作保持二叉查找树的平衡,是2-3-4树的一种,通过牺牲一定的平衡来达到删除插入时少量的旋转
...
每个红节点的子节点都是黑色(从每个叶子到根的所有路径上不能有连续的红色节点)

从任一节点到叶子节点的所有路径上包含相同数目的黑色节点
